    ...
    void Update()
    {
        ...
        if (!warp)
        {
            ...
            if (m_WrapMode == WrapMode.Loop)
                ...
                time = Mathf.PingPong(m_AutomaticTime, m_Meshes.Length - 1+0.3f);
            ...
            {
                warp = true;
                if(manger.GetComponent<valves>().outline == false)
                {
                    manger.GetComponent<valves>().outline = true;
                }
            }
            SetMorph(time);
        }
        ...
        if (t > Pause)
        {
            t = 0;
            warp = false;
            if (manger.GetComponent<valves>().outline ==true)
            {
                manger.GetComponent<valves>().outline = false;
            }
        }
    }
    public void setM()
